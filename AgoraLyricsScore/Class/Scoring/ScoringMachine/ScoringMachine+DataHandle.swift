...
extension ScoringMachine { /** ui 位置 **/
    ...
    /// 根据`演唱得分`计算`音准线view`的在父视图中的Y坐标
    /// - Parameters:
    ///   - pitchScore: 演唱得分 [0, 100]
    ///   - viewHeight: 父视图的实际高度
    ///   - minPitch: 对应歌曲音高中的最小值
    ///   - maxPitch: 对应歌曲音高中的最大值
    ///   - standardPitchStickViewHeight: `音准线view`的高度
    /// - Returns:`音准线view`在父视图中的Y坐标值
//    func calculatedY(pitchScore: Float,
//                     viewHeight: CGFloat,
//                     minPitch: Double,
//                     maxPitch: Double,
//                     standardPitchStickViewHeight: CGFloat) -> CGFloat? {
...
//
//        /** 计算实际的渲染高度 **/
//        let rate = calculatePercentage(value: pitchScore)
//        let renderingHeight = viewHeight - extends
//
//        /** 计算距离 （从bottom到top） **/
//        let distance = extends/2 + (renderingHeight * rate)
...
//
//        if y.isNaN {
//            Log.errorText(text: "calculatedY result invalid pitchScore:\(pitchScore) viewHeight:\(viewHeight) minPitch:\(minPitch) maxPitch:\(maxPitch) standardPitchStickViewHeight:\(standardPitchStickViewHeight)", tag: logTag)
//            return nil
//        }
//        
//        return y
//    }
    ...
    private func calculatePercentage(value: Float) -> Double {
